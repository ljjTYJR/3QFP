...
class Config:
    def __init__(self):
        ...
        self.debug: bool = False # debug mode or not
        ...
        self.opt_cor_sdf_only: bool = False # only optimize the sdf grid or not
        self.cor_sdf_w_feature: bool = False # use the feature grid to correct the sdf grid or not
        self.octree_fea_combine: str = 'cat' # combine the octree feature and the input feature by 'cat' or 'add'

        # pos encoding
        self.fixed_enc_type: str = 'freq'
        self.n_bins: int = 16
        ...
        self.regularization_loss_on: bool = False
        self.weight_r: float = 0.1
    ...
    def load(self, config_file):
        ...
        self.debug = config_args["setting"]["debug"]
        ...
        # feature encoder
        self.fea_encoder_type = config_args["fea_encoding"]["type"]
        if self.fea_encoder_type not in ['fea_octree', 'fea_plane']:
            raise ValueError("fea_encoder_type should be one of ['fea_octree', 'fea_plane']")

        # octree
        self.tree_level_world = config_args["fea_encoding"]["octree"]["tree_level_world"]
        # the number of the total octree level (defining the world scale)
        self.tree_level_feat = config_args["fea_encoding"]["octree"]["tree_level_feat"]  # the number of the octree level used for storing feature grid
        self.leaf_vox_size = config_args["fea_encoding"]["octree"]["leaf_vox_size"]  # the size of the grid on octree's leaf level (unit: m)
        self.feature_dim = config_args["fea_encoding"]["feature_dim"]  # feature vector's dimension
        self.poly_int_on = config_args["fea_encoding"]["poly_int_on"]  # use polynomial or linear interpolation of feature grids
        self.octree_from_surface_samples = config_args["fea_encoding"]["octree"]["octree_from_surface_samples"]  # build the octree from the surface samples or only the measurement points
        self.opt_cor_sdf_only = config_args["fea_encoding"]["prior_estimation"]["opt_cor_sdf_only"] # only optimize the sdf grid or not
        self.cor_sdf_w_feature = config_args["fea_encoding"]["prior_estimation"]["cor_sdf_w_feature"] # use the feature grid to correct the sdf grid or not
        self.coarse_compute = config_args["fea_encoding"]["prior_estimation"]["coarse_compute"] # str variable
        if self.coarse_compute not in ['Average', 'NDT', 'Nearest', 'Project']:
            raise ValueError("coarse_compute should be one of ['Average', 'NDT', 'Nearest', 'Project']")
        else:
            print("Use coarse compute: {}".format(self.coarse_compute))
        if self.cor_sdf_w_feature:
            print("Use optimizable sdfs and feature encodings")
        elif self.opt_cor_sdf_only:
            print("Use optimizable sdfs only")
        if (self.cor_sdf_w_feature and self.opt_cor_sdf_only):
            raise ValueError("cor_sdf_w_feature and opt_cor_sdf_only cannot be both true")
        if (not self.opt_cor_sdf_only and not self.cor_sdf_w_feature):
            print("No prior estimation is set")
        self.octree_fea_combine = config_args["fea_encoding"]["fea_octree"]["octree_fea_combine"]

        # pos encoding
        self.use_fixed_encoding = config_args["fea_encoding"]["use_fixed_encoding"]
        self.fixed_enc_type = config_args["fea_encoding"]["fixed_encoding"]["fixed_enc_type"]
        if self.use_fixed_encoding:
            print("Use pose encoding")
        self.n_bins = config_args["fea_encoding"]["fixed_encoding"]['n_bins']
        self.n_freq = config_args["fea_encoding"]["fixed_encoding"]['n_freq']
        self.gaussian_scale = config_args["fea_encoding"]["fixed_encoding"]['gaussian_scale']
        self.embedding_size = config_args["fea_encoding"]["fixed_encoding"]['embedding_size']

        self.use_features = config_args["fea_encoding"]["use_features"]
        self.progressive = config_args["fea_encoding"]["progressive"]
        ...
        self.behind_dropoff_on = config_args["loss"]["behind_dropoff_on"]  # apply "behind the surface" loss weight drop-off or not
        ...
        self.ekional_loss_on = config_args["loss"]["ekional_loss_on"]  # use ekional loss (gradient = 1 loss)
        self.weight_e = float(config_args["loss"]["weight_e"])
        self.regularization_loss_on = config_args["fea_encoding"]["fea_plane"]["regularization_loss_on"]
        self.weight_r = float(config_args["fea_encoding"]["fea_plane"]["weight_r"])
        # Only for debug
        self.record_feature_loss = config_args["fea_encoding"]["fea_plane"]["record_feature_loss"]
        ...
        self.lr_decay_step = config_args["optimizer"]["lr_decay_step"]
