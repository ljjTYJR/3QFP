...
class Mesher():
    ...
    def __init__(self, config: Config, fea_enc, \
        geo_decoder: Decoder, sem_decoder: Decoder):

        self.config = config

        self.fea_enc = fea_enc
    ...
    def query_points(self, coord, bs, query_sdf = True, query_sem = False, query_mask = True):
        ...
        check_level = min(self.fea_enc.featured_level_num, self.config.mc_vis_level)-1
        if query_sdf:
            ...
            if iter_n > 1:
                for n in tqdm(range(iter_n)):
                    ...
                    if self.opt_cor_sdf_only:
                        batch_sdf = -self.fea_enc.query_feature(batch_coord, False)
                    elif self.cor_sdf_w_feature:
                        batch_feature, batch_cor_sdf = self.fea_enc.query_feature(batch_coord, False) # query features
                        batch_res_sdf = -self.geo_decoder.sdf(batch_feature)
                        batch_sdf = batch_res_sdf + (-batch_cor_sdf) # `-` means the opposite sign
                    else:
                        batch_feature = self.fea_enc.query_feature(batch_coord, False) # query features
                        if query_sdf:
                    ...
                    if query_mask:
                        ...
                        if self.config.fea_encoder_type == 'fea_octree':
                            check_level_indices = self.fea_enc.hierarchical_indices[check_level]
                            mask_mc = check_level_indices >= 0
                            mask_mc = torch.all(mask_mc, dim=1)
                        elif self.config.fea_encoder_type == 'fea_plane':
                            # For feature planes, as long as one of the plane indices are `-1`, then the point is not valid
                            xy_indices = self.fea_enc.hier_plane_indices[check_level][0]; xy_mask = xy_indices >= 0
                            xz_indices = self.fea_enc.hier_plane_indices[check_level][1]; xz_mask = xz_indices >= 0
                            yz_indices = self.fea_enc.hier_plane_indices[check_level][2]; yz_mask = yz_indices >= 0
                            xy_mask_c = torch.all(xy_mask, dim=1)
                ...
                feature = self.fea_enc.query_feature(coord, False)
                if self.opt_cor_sdf_only:
                    sdf_pred = self.fea_enc.query_feature(coord, False).detach().cpu().numpy()
                elif self.cor_sdf_w_feature:
                    feature, cor_sdf = self.fea_enc.query_feature(coord, False)
                    res_sdf = -self.geo_decoder.sdf(feature)
                    sdf_pred = (res_sdf + (-cor_sdf)).detach().cpu().numpy()
                else:
                    if query_sdf:
                        if not self.config.time_conditioned:
    ...
    def recon_octree_mesh(self, query_level, mc_res_m, mesh_path, map_path, \
        ...
        nodes_coord_scaled = self.fea_enc.get_octree_nodes(query_level) # query level top-down
