...
def setup_experiment(config: Config):
    ...
    encoding_type = config.fea_encoder_type
    begin_frame = str(config.begin_frame)
    end_frame = str(config.end_frame)
    frame_step = str(config.every_frame)

    run_name = config.name + "_" + ts + '_' + encoding_type + '_' + begin_frame + '_' + end_frame + '_' + frame_step   # modified to a name that is easier to index
...
def setup_optimizer(config: Config, fea_enc_param, mlp_geo_param, mlp_sem_param, sigma_size) -> Optimizer:
    lr_cur = config.lr
    opt_setting = []

    if config.opt_cor_sdf_only:
        sdf_opt_dict = {'params': fea_enc_param[0], 'lr': lr_cur}
        lr_cur *= config.lr_level_reduce_ratio
        opt_setting.append(sdf_opt_dict)
    elif config.cor_sdf_w_feature:
        ...
        # feature octree
        for i in range(config.tree_level_feat + 1):
            # try to also add L2 regularization on the feature octree (results not quite good)
            feat_opt_dict = {'params': fea_enc_param[config.tree_level_feat-i], 'lr': lr_cur}
            lr_cur *= config.lr_level_reduce_ratio
        ...
        if config.fea_encoder_type == 'fea_octree':
            ...
            for i in range(config.tree_level_feat):
                # try to also add L2 regularization on the feature octree (results not quite good)
                feat_opt_dict = {'params': fea_enc_param[config.tree_level_feat-i-1], 'lr': lr_cur}
                lr_cur *= config.lr_level_reduce_ratio
                opt_setting.append(feat_opt_dict)
            # make sigma also learnable for differentiable rendering (but not for our method)
            if config.ray_loss:
                sigma_opt_dict = {'params': sigma_size, 'lr': config.lr}
                opt_setting.append(sigma_opt_dict)
        elif config.fea_encoder_type == 'fea_plane':
            if mlp_geo_param is not None:
                mlp_geo_param_opt_dict = {'params': mlp_geo_param, 'lr': lr_cur, 'weight_decay': config.weight_decay}
                opt_setting.append(mlp_geo_param_opt_dict)
            if config.semantic_on and mlp_sem_param is not None:
                mlp_sem_param_opt_dict = {'params': mlp_sem_param, 'lr': lr_cur, 'weight_decay': config.weight_decay}
                opt_setting.append(mlp_sem_param_opt_dict)
            for i in range(len(fea_enc_param)):
                # try to also add L2 regularization on the feature octree (results not quite good)
                feat_opt_dict = {'params': fea_enc_param[len(fea_enc_param) -i -1], 'lr': lr_cur}
                lr_cur *= config.lr_level_reduce_ratio
                opt_setting.append(feat_opt_dict)
...
def setup_optimizer_traj(config: Config, fea_enc_param, mlp_geo_param, mlp_sem_param, mlp_traj_param, sigma_size) -> Optimizer:
    lr_cur = config.lr
    opt_setting = []
    # weight_decay is for L2 regularization, only applied to MLP
    if mlp_geo_param is not None:
...
def save_checkpoint(
    fea_encoder, geo_decoder, run_path, checkpoint_name, iters
):
    torch.save(
        {
            "iters": iters,
            "feature_enc": fea_encoder, # save the whole NN module (the hierachical features and the indexing structure)
            "geo_decoder": geo_decoder.state_dict(),
            # "sem_decoder": sem_decoder.state_dict(),
            # I do not need to save the optimizer, I do not want to continue training
