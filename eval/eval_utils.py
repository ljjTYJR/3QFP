...
def eval_mesh(file_pred, file_trgt, down_sample_res=0.02, threshold=0.05, truncation_acc=0.50, truncation_com=0.50, gt_bbx_mask_on= True,
    ...
    # visualize the predicted mesh
    """
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    ctr = vis.get_view_control()
    parameters = o3d.io.read_pinhole_camera_parameters("ScreenCamera_2023-09-11-10-54-58.json")
    ...
    _, _, dist_p = nn_correspondance(verts_trgt, verts_pred, truncation_acc, True) # find nn in ground truth samples for each predict sample -> precision related
    _, _, dist_r = nn_correspondance(verts_pred, verts_trgt, truncation_com, False) # find nn in predict samples for each ground truth sample -> recall related
    ...
    # visualize the mesh and the outliers
    """
    verts_morethan_threshold = verts_trgt[dist_r > threshold]
    pcd_tgt_morethan_threshold = o3d.geometry.PointCloud()
    pcd_tgt_morethan_threshold.points = o3d.utility.Vector3dVector(verts_morethan_threshold)
    # orange
    pcd_tgt_morethan_threshold.paint_uniform_color([1, 0.75, 0])
    # normals
    mesh_pred.compute_vertex_normals()
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    ctr = vis.get_view_control()
    parameters = o3d.io.read_pinhole_camera_parameters("ScreenCamera_2023-09-10-12-32-11.json")
    vis.add_geometry(mesh_pred)
    # vis.add_geometry(pcd_tgt_morethan_threshold)
    ctr.convert_from_pinhole_camera_parameters(parameters)
    vis.run()
    vis.destroy_window()
    """
    ...
    for idx, vert in enumerate(verts2):
        ...
        if dist_square[0] < truncation_dist_square:
            ...
            if not ignore_outlier:
                no_match_indices_v2.append(idx)
                indices.append(inds[0])
                distances.append(truncation_dist)

    return no_match_indices_v2, indices, distances
...
# # make the cropped ground truth point cloud for accuracy evaluation

# gt_pcd_path = "/media/shuo/T7/mai_city/mai_city/ply/sequences/02/gt/0_100_1.ply"
# files_pred = [
#     "/media/shuo/T7/experiment_result/mai_city/dense_input/vdbfusion/mesh0.ply",
#     "/media/shuo/T7/experiment_result/mai_city/dense_input/shine_mapping/mesh_iter_50000.ply",
#     "/media/shuo/T7/experiment_result/mai_city/dense_input/ours/mesh_iter_50000.ply"
# ]
# out_file_crop = "/media/shuo/T7/mai_city/mai_city/ply/sequences/02/gt/0_100_1_crop_fro_acc.ply"
# crop_intersection(gt_pcd_path, files_pred, out_file_crop, dist_thre=0.2, mesh_sample_point=1000000)
