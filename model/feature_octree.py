...
from model.encodings import FrequencyEncoder, tiny_cuda_nn_get_encoder
...
class FeatureOctree(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.opt_cor_sdf_only = config.opt_cor_sdf_only
        self.cor_sdf_w_feature = config.cor_sdf_w_feature
        ...
        # The optimizable SDF parameters storing the corner SDF value in the finest level
        self.finest_sdf_features = None
        self.coarse_compute = config.coarse_compute
        self.octree_fea_combine = config.octree_fea_combine # The way of combining the octree features, 'cat' or 'add'

        # The pos encoding with normalization coordinates
        ...
        if self.use_features:
            if self.octree_fea_combine == 'cat':
                self.octree_fea_dim = self.feature_dim * self.featured_level_num
            elif self.octree_fea_combine == 'add':
                self.octree_fea_dim = self.feature_dim
        self.out_dim = self.input_ch_pos + self.octree_fea_dim
    ...
    # FIXME The `surface points` are already transformed to the global frame, not just a local frame where the origin is (0,0,0)
    def update(self, surface_points, incremental_on = False, origin = None):
        # [0 1 2 3 ... max_level-1 max_level]
        pcd_sur = o3d.geometry.PointCloud()
        pcd_sur.points = o3d.utility.Vector3dVector(surface_points.cpu().numpy())
        pcd_sur.paint_uniform_color([1, 0.706, 0])
        pcd_sur_tree = o3d.geometry.KDTreeFlann(pcd_sur)
        ...
        for i in range(self.max_level+1): # for each level (top-down)
            ...
            if len(self.corners_lookup_tables[i]) == 0: # for the first frame
                ...
                # initializa corner features, TODO: change to the explicit SDF value and construct another optimizable features.
                ...
                if (i == self.max_level) and (self.opt_cor_sdf_only or self.cor_sdf_w_feature):
                    # TODO Currently, use the first frame to initialize the SDF value. Consider integration later
                    if self.coarse_compute == 'Average':
                        fea = spc.features[new_nodes_index]
                        corners_nor = (corners.reshape(-1, 8, 3) / (2 ** i)) * 2 - 1
                        signs = -torch.sign(torch.einsum('ijk,ik->ij', fea[:, None, :] - corners_nor, fea - origin))[..., None]
                        dist = torch.linalg.norm(corners_nor - fea[:, None, :], dim=2, keepdim=True)
                        sdf_estimated = (dist * signs).reshape(-1)
                        _, cor_mor_uni_idx = torch.unique(corners, dim=0, return_inverse=True)
                        vsdf = scatter_mean(sdf_estimated, cor_mor_uni_idx)[:,None]
                    elif self.coarse_compute == 'Nearest':
                        # TODO check whether the voxel size is correct or not
                        voxel_size = 1 / (2 ** (i-1))
                        search_dist = np.sqrt(6) * voxel_size
                        corner_uni_nor = ((corners_unique / (2 ** i)) * 2 - 1).cpu().numpy() #N*3
                        # search_hybrid_vector_3d
                        search_idx = []
                        for j in range(corner_uni_nor.shape[0]):
                            [k, idx, _] = pcd_sur_tree.search_knn_vector_3d(corner_uni_nor[j], 3)
                            search_idx.append(idx[1])
                        search_idx = np.array(search_idx)
                        searched_points = np.array(pcd_sur.points)[search_idx] # N*3
                        origin_np = np.array(origin.cpu())
                        signs = -np.sign(np.einsum('ij,ij->i', searched_points - corner_uni_nor, searched_points - origin_np))[..., None]
                        dist = np.linalg.norm(searched_points - corner_uni_nor, axis=1, keepdims=True)
                        sdf_estimated = (dist * signs).reshape(-1)
                        vsdf = torch.tensor(sdf_estimated, device=self.device)[:,None]

                    self.finest_sdf_features = nn.Parameter(vsdf)

            else: # update for new frames
                pre_size = len(self.corners_lookup_tables[i])
                new_corner_idx = []
                for idx, m in enumerate(corners_morton):
                    if m not in self.corners_lookup_tables[i]: # add new keys
                        self.corners_lookup_tables[i][m] = len(self.corners_lookup_tables[i])
                        new_corner_idx.append(idx)
                ...
                if (i == self.max_level) and (self.opt_cor_sdf_only or self.cor_sdf_w_feature):
                    if self.coarse_compute == 'Average':
                        ...
                        sdf_estimated = (dist * signs).reshape(-1)
                        vsdf = torch.tensor(sdf_estimated, device=self.device)[:,None]
                    self.finest_sdf_features = nn.Parameter(torch.cat((self.finest_sdf_features, nn.Parameter(vsdf)),0))
    ...
    def get_indices(self, coord):
        ...
        for i in range(self.featured_level_num): # bottom-up, for each level
            ...
            # NOTE: look up the voxel's 8 corner nodes' indices in the look up table, if not found, assign -1?
    ...
    def query_feature_with_indices(self, coord, hierarchical_indices):
        sum_features = torch.zeros(coord.shape[0], self.out_dim, device=self.device)
        if self.use_fixed_encoding and self.use_features:
            for i in range(self.featured_level_num): # for each level
                current_level = self.max_level - i
                feature_level = self.featured_level_num-i-1 # bottom-up
                # Interpolating
                ...
                if self.octree_fea_combine == 'cat':
                    sum_features[:, feature_level*self.feature_dim:(feature_level+1)*self.feature_dim] = (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).sum(1)
                elif self.octree_fea_combine == 'add':
                    sum_features[:, :self.feature_dim] += (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).sum(1)
            ...
            pos_enc = self.pos_encoder(coord)
            ...
            raise ValueError('No feature is used!')

        # If only optimize the SDF values in corners
        sdf_inter_values = torch.zeros(coord.shape[0], 1, device=self.device)

        if self.opt_cor_sdf_only:
            # only using the finest level SDF values
            current_level = self.max_level
            feature_level = self.featured_level_num-1
            # Interpolating
            coeffts = self.interpolat(coord,current_level,self.polynomial_interpolation)
            sdf_inter_values = (self.finest_sdf_features[hierarchical_indices[feature_level]]*coeffts).sum(1)
            sdf_inter_values = sdf_inter_values.squeeze()
            return sdf_inter_values

        if self.cor_sdf_w_feature:
            # only using the finest level SDF values
            current_level = self.max_level
            feature_level = self.featured_level_num-1
            # Interpolating
            ...
            sdf_inter_values = (self.finest_sdf_features[hierarchical_indices[feature_level]]*coeffts).sum(1)
            sdf_inter_values = sdf_inter_values.squeeze()
            return sum_features, sdf_inter_values
    ...
    def query_feature(self, coord, faster = False):
        ...
        # NOTE: The indices are grouped into 3 levels, each level has a list of indices for the batch points
        if self.cor_sdf_w_feature:
            features, sdf_inter_values = self.query_feature_with_indices(coord, indices)
            return features, sdf_inter_values
        else:
