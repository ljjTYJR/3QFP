...
class LiDARDataset(Dataset):
    def __init__(self, config: Config, fea_encoder) -> None:
    ...
    def process_frame(self, frame_id, incremental_on = False):
        ...
        if self.fea_encoder is not None:
            if self.config.octree_from_surface_samples:
                self.fea_encoder.update(coord[weight > 0, :].to(self.device), incremental_on, origin=frame_origin_torch, )
            else:
                self.fea_encoder.update(frame_pc_s_torch.to(self.device), incremental_on, origin=frame_origin_torch, )
    ...
    def get_batch(self):
        ...
        if self.config.ray_loss:
            ...
            # NOTE: generate random index on the cpu and then copy to the gpu (low, high, batch_size)
